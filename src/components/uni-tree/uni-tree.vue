<template>
  <view class="next-tree">
    <view
      class="next-tree-mask"
      :class="showTree ? 'show' : ''"
      @tap="_cancel"
    ></view>
    <view class="next-tree-cnt" :class="showTree ? 'show' : ''">
...
export default {
    ...
    confirmEach (list) {
      list.forEach((item, index) => {
        ...
        if (item.children && item.children.length > 0)
          this.confirmEach(item.children);
      });
    },
    //扁平化树结构
    ...
    setAncestors (pids, checked) {
      this.treeList.forEach((item, index) => {
        if (pids.includes(item.id)) {
          if (
            checked &&
            this.childNums[item.id] !== undefined &&
            item.childNum === this.childNums[item.id]
          ) {
            // 子级全部选中, 父级才选中
  ...
  mounted () {
    ...
    this.$watch(
      () => [this.multiple, this.selectParent],
      () => {
